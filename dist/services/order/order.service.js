...
exports.getOrderDetailAdminService = exports.getAllOrdersAdminService = exports.cancelOrderService = exports.getOrderDetailByUserService = exports.getOrdersByUserService = void 0;
 ...
 */
const getOrdersByUserService = async (userId, query) => {
    const page = Math.max(1, Number(query.page || 1));
    const size = Math.max(1, Number(query.pageSize || constant_js_1.pageSize));
    const skip = (page - 1) * size;
    const now = new Date();
    const expiryThreshold = new Date(now.getTime() - constant_js_1.ORDER_PAYMENT_TIMEOUT_MINUTES * 60 * 1000);
    // Tự động xóa mềm các đơn hàng chưa thanh toán của user này nếu đã quá 15 phút
    await client_js_1.prisma.order.updateMany({
        where: {
            user_id: userId,
            payment_status: { in: [constant_js_1.PaymentStatus.UNPAID, constant_js_1.PaymentStatus.FAILED] },
            deleted_at: null,
    ...
    const [total, orders] = await client_js_1.prisma.$transaction([
        client_js_1.prisma.order.count({ where }),
        client_js_1.prisma.order.findMany({
            where,
            skip,
            take: size,
    ...
    const formattedOrders = orders.map((order) => {
        const isPaid = order.payment_status === constant_js_1.PaymentStatus.PAID;
        const expiresAt = new Date(order.created_at.getTime() + constant_js_1.ORDER_PAYMENT_TIMEOUT_MINUTES * 60 * 1000);
 ...
 */
const getOrderDetailByUserService = async (userId, identifier) => {
    ...
    const order = await client_js_1.prisma.order.findFirst({
        where,
        include: {
            order_details: {
                include: {
    ...
    const formattedOrderDetails = order.order_details.map((detail) => {
        ...
        const matchingAppointments = order.appointments.filter((apt) => apt.package_id === detail.package_id && apt.status !== constant_js_1.AppointmentStatus.CANCELLED);
    ...
    if (isExpired && order.deleted_at === null) {
        ...
        order.payment_status = constant_js_1.PaymentStatus.FAILED;
...
exports.getOrderDetailByUserService = getOrderDetailByUserService;
/**
 * Hủy đơn hàng chưa thanh toán (UNPAID)
 * - Chỉ cho phép hủy khi đơn hàng thuộc về user và đang ở trạng thái UNPAID
 */
const cancelOrderService = async (userId, identifier) => {
    const isNumeric = typeof identifier === 'number' || /^\d+$/.test(String(identifier));
    const where = {
        user_id: userId,
        ...(isNumeric ? { id: Number(identifier) } : { order_code: String(identifier) }),
    ...
    const updatedOrder = await client_js_1.prisma.order.update({
        where: { id: order.id },
        data: {
            deleted_at: new Date(),
            payment_status: constant_js_1.PaymentStatus.FAILED,
 ...
 */
const getAllOrdersAdminService = async (query) => {
    const page = Math.max(1, Number(query.page || 1));
    const size = Math.max(1, Number(query.pageSize || constant_js_1.pageSize));
    const skip = (page - 1) * size;
    const now = new Date();
    ...
    if (query.status) {
        ...
        if (upperStatus === constant_js_1.PaymentStatus.CANCELLED) {
...
exports.getAllOrdersAdminService = getAllOrdersAdminService;
/**
 ...
 */
const getOrderDetailAdminService = async (identifier) => {
    const isNumeric = typeof identifier === 'number' || /^\d+$/.test(String(identifier));
    const where = isNumeric ? { id: Number(identifier) } : { order_code: String(identifier) };
    const order = await client_js_1.prisma.order.findFirst({
        where,
        include: {
            user: {
