...
import { CreateArticleData, GetArticlesOptions, UpdateArticleData } from '../../types/article/Article.js';
...
export const getArticles = async (options: GetArticlesOptions) => {
  const { page = 1, pageSize = defaultPageSize, search, specialty_id, author_id, status, showDeleted = false } = options;
...
export const getArticleBySlug = async (slugOrId: string) => {
  const isNumeric = !isNaN(Number(slugOrId));
  const where: Prisma.ArticleWhereInput = isNumeric
...
export const createArticle = async (data: CreateArticleData) => {
  ...
  const existingArticleWithTitle = await prisma.article.findFirst({
    where: {
      title: trimmedTitle,
      deleted_at: null,
    },
...
export const updateArticle = async (id: number, data: UpdateArticleData, authorIdContext?: number) => {
  ...
  if (data.title && data.title.trim() !== existingArticle.title) {
    ...
    let checkSlug = await prisma.article.findFirst({
      where: { slug: newSlug, id: { not: id } }
    });
    let counter = 1;
    while (checkSlug) {
      newSlug = `${generateSlug(trimmedTitle)}-${counter}`;
      checkSlug = await prisma.article.findFirst({
        where: { slug: newSlug, id: { not: id } }
      });
      counter++;
    }
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (data.thumbnail_url !== undefined) articleData.thumbnail_url = data.thumbnail_url;
    if (data.short_description !== undefined) articleData.short_description = data.short_description;
