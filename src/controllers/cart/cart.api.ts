...
import {
    ...
    getAllCartsAdminService,
    getCartByUserService,
    getCartDetailAdminService,
    updateCartQuantityService,
} from '../../services/cart/cart.service.js';
import {
    AddToCartSchema,
    UpdateCartQuantitySchema,
} from '../../model/Schema/Cart/Cart_Schema.js';
import { parsePriceRanges, RoleType } from '../../config/constant.js';
 ...
 */
export const getCartsAPI = async (req: Request, res: Response) => {
    try {
        const isAdminRoute =
            req.baseUrl.includes('/admin') ||
            req.originalUrl.includes('/admin') ||
            (req.user?.role === RoleType.ADMIN && req.path === '/carts');

        if (isAdminRoute && req.user?.role === RoleType.ADMIN) {
            const {
                page,
                search,
                min_price,
            ...
            const parsedRanges = parsePriceRanges(price_range || price_ranges);

            const data = await getAllCartsAdminService({
        ...
        const data = await getCartByUserService(Number(userId));
 ...
 */
export const postCartsAPI = async (req: Request, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
        ...
        const parsed = await AddToCartSchema.safeParseAsync(req.body);
        if (!parsed.success) {
        ...
        const data = await addToCartService(Number(userId), parsed.data);
 ...
 */
export const updateCartQuantityAPI = async (req: Request, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
        ...
        const parsed = await UpdateCartQuantitySchema.safeParseAsync(req.body);
        if (!parsed.success) {
            const errors = parsed.error.issues.map(
                (i) => `${i.message} (${i.path[0]?.toString()})`
            );
 ...
 */
export const deleteCartsAPI = async (req: Request, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
        ...
        if (!rawPackageId || rawPackageId === 'all' || rawPackageId === 'clear') {
            const result = await clearCartService(Number(userId));
            return res.status(200).json({
                status: 'success',
        ...
        const result = await deleteCartItemService(Number(userId), packageId);
 ...
 */
export const getCartDetailAdminAPI = async (req: Request, res: Response) => {
    try {
        ...
        const data = await getCartDetailAdminService(identifier);
