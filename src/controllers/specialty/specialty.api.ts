...
} from '../../services/specialty/specialty.service.js';
import {
    CreateSpecialtySchema,
    UpdateSpecialtySchema,
} from '../../model/Schema/Specialty/Specialty_Schema.js';
 ...
 */
export const getSpecialtiesDetailAPI = async (req: Request, res: Response) => {
    try {
        ...
        const isAdmin = req.user?.role === RoleType.ADMIN;
        const data = await getSpecialtyByIdService(specialtyId, isAdmin);
 ...
 */
export const postSpecialtyAPI = async (req: Request, res: Response) => {
    try {
        const parsed = await CreateSpecialtySchema.safeParseAsync(req.body);
        if (!parsed.success) {
        ...
        const data = await createSpecialtyService(parsed.data);
 ...
 */
export const updateSpecialtyAPI = async (req: Request, res: Response) => {
    try {
        const specialtyId = Number(req.params.id);
        if (!specialtyId || isNaN(specialtyId)) {
            return res.status(400).json({
        ...
        const parsed = await UpdateSpecialtySchema.safeParseAsync(req.body);
        if (!parsed.success) {
            const errors = parsed.error.issues.map(
                (i) => `${i.message} (${i.path[0]?.toString()})`
            );
        ...
        const data = await updateSpecialtyService(specialtyId, parsed.data);
 ...
 */
export const deleteSpecialtyAPI = async (req: Request, res: Response) => {
    try {
 ...
 */
export const restoreSpecialtyAPI = async (req: Request, res: Response) => {
    try {
        const specialtyId = Number(req.params.id || req.body.id || req.query.id);
        if (!specialtyId || isNaN(specialtyId)) {
            return res.status(400).json({
        ...
        await restoreSpecialtyService(specialtyId);
